...
export type SfxName = 'pop' | 'whoosh' | 'sparkle' | 'chime' | 'tick'

export interface SfxOptions {
  /** 0 → 1. Drives the countdown tick's pitch, so it climbs toward zero. */
  pitch?: number
}
...
export function createAudioEngine(): AudioEngine {
  ...
  function play(name: SfxName, opts?: SfxOptions) {
    if (!ctx || !sfxBus || !noise) return
    const t = ctx.currentTime

    if (name === 'tick') {
      // A countdown beat: bright blip that climbs in pitch as zero nears,
      // over a low thump so it lands with some weight.
      const p = Math.max(0, Math.min(1, opts?.pitch ?? 0))
      const base = 520 + p * 560

      const blip = ctx.createOscillator()
      blip.type = 'triangle'
      blip.frequency.setValueAtTime(base, t)
      blip.frequency.exponentialRampToValueAtTime(base * 1.45, t + 0.07)
      const bg = ctx.createGain()
      bg.gain.setValueAtTime(0, t)
      bg.gain.linearRampToValueAtTime(0.2 + p * 0.14, t + 0.006)
      ...
      tg.gain.setValueAtTime(0.26 + p * 0.1, t)
      tg.gain.exponentialRampToValueAtTime(0.0001, t + 0.2)
      thump.connect(tg).connect(sfxBus)
      thump.start(t)
      thump.stop(t + 0.22)

      // a crisp transient on the front of the beat
